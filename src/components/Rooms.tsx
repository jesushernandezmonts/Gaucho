    ...
    <section id="habitaciones" className="py-14 sm:py-20 md:py-28 bg-cream relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
        <div ref={headerRef} className={`text-center mb-8 sm:mb-14 ${headerClass}`}>
          ...
          <h2 className="font-display text-xl sm:text-3xl md:text-5xl font-bold text-chocolate leading-tight">Habitaciones con Alma</h2>
          <div className="flex items-center justify-center gap-2 sm:gap-3 mt-3 sm:mt-5">
            ...
            <svg className="w-3 sm:w-4 h-3 sm:h-4 text-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
            <span className="block w-8 sm:w-14 md:w-20 h-px sm:h-0.5 bg-gradient-to-r from-transparent via-gold to-transparent" />
          </div>
        </div>
          ...
          {rooms.map((room, i) => (
            <div
              ...
              className={`bg-white rounded-2xl overflow-hidden transition-all duration-500 group ${
                room.featured
                  ? "shadow-gold/20 shadow-xl hover:shadow-gold/30 hover:-translate-y-1"
            ...
            >
              ...
              <div className="p-4 sm:p-5 md:p-6">
                <div className="flex items-center justify-between mb-2 gap-2">
                  <h3 className="font-display text-base sm:text-xl font-bold text-chocolate truncate">{room.name}</h3>
                  {!room.featured && (
                    <span className={`shrink-0 text-[10px] font-bold uppercase tracking-wider px-2 sm:px-3 py-1 rounded-full ${room.badgeColor}`}>
                      {room.badge}
                    </span>
                  )}
                </div>

                <div className="flex flex-wrap gap-1.5 sm:gap-3 mb-2 sm:mb-3 text-[11px] sm:text-xs text-chocolate-light">
                ...
                <p className="text-xs sm:text-sm text-chocolate-light leading-relaxed mb-3 sm:mb-5 line-clamp-2 sm:line-clamp-none">{room.desc}</p>

                <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-chocolate/10">
                  <span className="font-display text-base sm:text-xl font-bold text-gold">
                    ${room.price.toLocaleString("es-MX")}{" "}
                    <span className="font-body text-[10px] sm:text-xs font-normal text-chocolate-light">/ noche</span>
                  </span>
                  <a
                    href="#reservaciones"
